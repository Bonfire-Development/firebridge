...
mixin class EventParser {
  GatewayEvent parseGatewayEvent(Map<String, Object?> raw,
    ...
    final mapping = {
      ...
      Opcode.heartbeatAck.value: (Map<String, Object?> raw) =>
          parseHeartbeatAck(raw,
              heartbeatLatency: heartbeatLatency ?? Duration.zero),
    };
  ...
  HelloEvent parseHello(Map<String, Object?> raw) {
    return HelloEvent(
      heartbeatInterval: Duration(
        milliseconds:
            (raw['d'] as Map<String, Object?>)['heartbeat_interval'] as int,
      ),
    );
  }

  HeartbeatAckEvent parseHeartbeatAck(Map<String, Object?> raw,
      {required Duration heartbeatLatency}) {
