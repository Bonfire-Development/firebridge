...
class HttpRoute {
  ...
  String get rateLimitId => parts
      .expand((part) => [
            part.name,
            ...part.params
                .map((param) => param.isMajor ? param.value : r'$param'),
          ])
      .join('/');
...
extension RouteHelpers on HttpRoute {
  ...
  void guilds({String? id}) => add(HttpRoutePart(
      "guilds", [if (id != null) HttpRouteParam(id, isMajor: true)]));
  ...
  void channels({String? id}) => add(HttpRoutePart(
      "channels", [if (id != null) HttpRouteParam(id, isMajor: true)]));
  ...
  /// Adds the [`acknowledge`](https://docs.discord.sex/resources/message#acknowledge-message) part to this [HttpRoute].
  void acknowledge() => add(HttpRoutePart("ack"));
